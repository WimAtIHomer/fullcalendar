...
function RoomEventRenderer() {
	...
	function compileSlotSegs(events) {
        ...
        colSegs = sliceSegs(
            events,
            t.start.clone().time(minTime),
            t.end.clone().time(maxTime)
        );

        for (c=0; c<colCnt; c++) {
            for (j=0; j<colSegs.length; j++) {
                if (colSegs[j].event.roomKey == rooms[c].key) {
                    seg = colSegs[j];
                    seg.col = c;
                    segs.push(seg);
                }
        ...
        segs = placeSlotSegs(segs); // returns a new order
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			seg = segs[i];
			event = seg.event;
			top = computeDateTop(seg.start, t.start);
			bottom = computeDateTop(seg.end, t.start);
...
function computeSlotSegCollisions(seg, otherSegs, results) {
	...
	for (var i=0; i<otherSegs.length; i++) {
		if (seg.col == otherSegs[i].col && isSlotSegCollision(seg, otherSegs[i])) {
